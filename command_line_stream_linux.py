...
if number_of_streams == "1":
    rtmp1 = input("Stream RTMP(s): ")
    key1 = input("Stream Key: ")
    encoder = f"-f video4linux2 -framerate 30 -video_size hd1080 -i {video_input} -f alsa -ac 2 -i hw:1 "
    ...
    #
    stream = f'ffmpeg -f video4linux2 -input_format mjpeg -video_size {resolution} -i {video_input} -thread_queue_size 2048 ' \
             f'-f alsa -ac 2 -i hw:2 -map 0 -flags +global_header -c:v libx264 -crf 20.0 -crf_max 25.0 -profile high444 ' \
             f'-tune fastdecode -preset veryfast -trellis 2 -maxrate 2600k -bufsize 5200k -pix_fmt yuv420p -g 50 ' \
             f'-map 1 -c:a aac -b:a 160k -ac 2 -ar 44100 -f tee "[f=flv]{rtmp1}/{key1}'
...
if number_of_streams == "2":
    ...
    stream = f'ffmpeg -f video4linux2 -input_format mjpeg -video_size {resolution} -i {video_input} -thread_queue_size 2048 ' \
         f'-f alsa -ac 2 -i hw:2 -map 0 -flags +global_header -c:v libx264 -crf 20.0 -crf_max 25.0 -profile high444 ' \
         f'-tune fastdecode -preset veryfast -trellis 2 -maxrate 2600k -bufsize 5200k -pix_fmt yuv420p -g 50 ' \
         f'-map 1 -c:a aac -b:a 160k -ac 2 -ar 44100 -f tee "[f=flv]{rtmp1}/{key1}|[f=flv]{rtmp2}/{key2}'
...
with open(f'{stream_name}.py', 'w') as f:
    f.write("import os\nfrom os.path import exists\ni = 0\nwhile os.path.exists(f'"f'/home/tyler/Desktop/stream_recordings/{stream_name}'"{i}.mp4'):\n    i += 1\nfilename = f'"f'{stream_name}'"{i}.mp4'\nstream = f'"f'{stream}'"|[f=flv]/home/tyler/Desktop/stream_recordings/{filename}"f'{ap}'f'{ap2}'"\ninput('Press enter to start stream, Ctrl-c to quit')\nos.system(stream)")

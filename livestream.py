...
#stream = f'ffmpeg -re -stream_loop -1 -i {path} -map 0 -flags +global_header -c:v libx264 -crf 20.0 -crf_max 25.0 -profile high444 -preset veryfast -trellis 2 -maxrate 2600k -bufsize 5200k -pix_fmt yuv420p -g 50 -c:a aac -b:a 160k -ac 2 -ar 44100 -f tee "[f=flv]{rtmp}/{key}|[f=flv]{stream_name}.mp4"'
